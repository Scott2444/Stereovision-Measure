...
class PixelOrthoDist(unittest.TestCase):
    def test_isoceles_triangle(self):
        ...
        calculations = ObjectCalculations(leftPredicitons=InstanceData(), rightPredictions=InstanceData(), frameWidth=640)
        target = 545.85 #  in mm
        self.assertAlmostEqual(calculations._calculate_point_distance(100, 540), target, delta=1)

    def test_scalene_triangle(self):
        # Left camera makes 85° interior angle, right camera makes 80° interior angle
        # 430 pixels on the right side, 100 pixels on the left side
        calculations = ObjectCalculations(leftPredicitons=InstanceData(), rightPredictions=InstanceData(), frameWidth=640)
        target = 729.67 #  in mm
        self.assertAlmostEqual(calculations._calculate_point_distance(100, 430), target, delta=1)
    ...
    def test_obtuse_triangle(self):
        ...
        self.assertAlmostEqual(calculations._calculate_point_distance(100, 210), target, delta=1)
